//took help from the code from: https://github.com/ml5js/ml5-library/blob/release/src/KNNClassifier/index.js

import * as tf from "@tensorflow/tfjs";
import * as knnClassifier from "@tensorflow-models/knn-classifier";
import { callCallback, loadFile, saveBlob } from "./model-utils";
...
class KNN {
  ...
  async save(name) {
    ...
    await saveBlob(
      JSON.stringify({ dataset, tensors }),
      fileName,
  ...
  async load(pathOrData, callback) {
    ...
    if (typeof pathOrData === "object") {
      ...
      data = await loadFile(pathOrData);
    }
    if (data) {
